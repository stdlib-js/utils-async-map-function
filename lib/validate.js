...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '1RN2V,FD', options ) );
	}
	...
	if ( hasOwnProp( options, 'series' ) ) {
		...
		if ( !isBoolean( opts.series ) ) {
			return new TypeError( format( '1RN2o,GE', 'series', opts.series ) );
		}
	}
	if ( hasOwnProp( options, 'limit' ) ) {
		opts.limit = options.limit;
		if ( !isPositiveInteger( opts.limit ) ) {
			return new TypeError( format( '1RN3P,Fv', 'limit', opts.limit ) );
		}
	}
	return null;
}
